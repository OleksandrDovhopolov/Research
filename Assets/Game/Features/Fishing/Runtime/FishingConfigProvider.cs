...
namespace Game.Fishing
{
    ...
    public sealed class JsonFishingConfigProvider : IFishingConfigProvider
    {
        ...
        public async UniTask<FishingStaticData> LoadAsync(CancellationToken ct)
        {
            ...
            var fishRoot = await LoadAsync<FishConfigRootDto>(FishingConfigPaths.Fish, ct);
            var zonesRoot = await LoadAsync<FishingZonesConfigRootDto>(FishingConfigPaths.Zones, ct);
            var luresRoot = await LoadAsync<LuresConfigRootDto>(FishingConfigPaths.Lures, ct);
            var itemsRoot = await LoadAsync<FishingItemsConfigRootDto>(FishingConfigPaths.Items, ct);
            var settingsRoot = await LoadAsync<FishingSettingsConfigRootDto>(FishingConfigPaths.Settings, ct);
            var recipeRefsRoot = await LoadAsync<FishingCraftingRecipesRefRootDto>(FishingConfigPaths.CraftingRecipes, ct);

            var staticData = FishingConfigMapper.ToStaticData(
                fishRoot,
                zonesRoot,
                luresRoot,
                itemsRoot,
