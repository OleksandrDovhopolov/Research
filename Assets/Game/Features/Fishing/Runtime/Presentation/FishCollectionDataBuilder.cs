...
namespace Game.Fishing
{
    ...
    public sealed class FishCollectionDataBuilder : IFishCollectionDataBuilder
    {
        ...
        public async UniTask<FishCollectionArgs> BuildAsync(CancellationToken ct = default)
        {
            ...
            var waterBodyTypesById = (data?.WaterBodyTypes ?? new List<WaterBodyTypeConfig>())
                .Where(x => x != null && !string.IsNullOrWhiteSpace(x.Id))
                .GroupBy(x => x.Id, StringComparer.Ordinal)
                .ToDictionary(group => group.Key, group => group.Last(), StringComparer.Ordinal);
            var luresById = (data?.Lures ?? new List<LureConfig>())
                .Where(x => x != null && !string.IsNullOrWhiteSpace(x.Id))
                .GroupBy(x => x.Id, StringComparer.Ordinal)
                .ToDictionary(group => group.Key, group => group.Last(), StringComparer.Ordinal);
            var fishList = data?.Fish ?? new List<FishConfig>();
